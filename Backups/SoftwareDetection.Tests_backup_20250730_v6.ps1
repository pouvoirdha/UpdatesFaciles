...
Describe "SoftwareDetection Module Tests" {
    BeforeAll {
        # Mock pour Get-ItemProperty (registre, uniquement pour *Uninstall\*)
        ...
        # Mock par défaut pour tout autre appel à Get-ItemProperty
        Mock Get-ItemProperty {
            return @()
        ...
        } -ModuleName SoftwareDetection -ParameterFilter { $Path -like "*Desktop*" -or $Path -like "*Start Menu*" }
        ...
        # Mock pour New-SoftwareApp avec paramètres nommés
        ...
        # Mock pour Remove-DuplicateSoftware pour éviter l'erreur critique
        Mock Remove-DuplicateSoftware {
            param(
                [array]$SoftwareList
            )
            # Retourne la liste d'entrée si non vide, sinon une liste vide
            if ($SoftwareList) {
                return $SoftwareList
            }
            return @()
