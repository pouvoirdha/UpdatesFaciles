...
Describe "SoftwareDetection Module Tests" {
    BeforeAll {
        ...
        } -ParameterFilter { $Path -eq "D:\Portable" }
        ...
        } -ParameterFilter { $Path -like "*Desktop*" }

        Mock New-Object {
            return [PSCustomObject]@{ CreateShortcut = { return [PSCustomObject]@{ TargetPath = "C:\Program Files\TestApp\TestApp.exe" } } }
        } -ParameterFilter { $TypeName -eq "WScript.Shell" }

        # Mock pour Get-FileVersionSafely
        Mock -CommandName Get-FileVersionSafely -ModuleName SoftwareDetection {
            return @{
                ProductName = "TestApp"
                ProductVersion = "1.0"
                CompanyName = "TestCorp"
            }
        } -ParameterFilter { $FilePath -like "*TestApp.exe" }
    }
    ...
    Context "Get-InstalledSoftware" {
        ...
        It "Gère les erreurs sans interrompre" {
            Mock Get-ItemProperty { throw "Erreur registre" } -ParameterFilter { $Path -like "*Uninstall\*" }
            $result = Get-InstalledSoftware
            $result | Should -Be @()
        }
    }
    ...
    Context "Get-ShortcutSoftware" {
        ...
        It "Gère les erreurs de COM sans interrompre" {
            Mock New-Object { throw "Erreur COM" } -ParameterFilter { $TypeName -eq "WScript.Shell" }
            $result = Get-ShortcutSoftware
            $result | Should -Be @()
        }
    }
