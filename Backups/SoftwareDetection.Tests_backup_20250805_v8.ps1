...
Describe "SoftwareDetection Module Tests" {
    BeforeAll {
        ...
        Mock Get-ItemProperty {
            Write-Debug "Mock Get-ItemProperty appelé pour Path: $Path"
            return @(
                [PSCustomObject]@{
                    DisplayName = "TestApp"
                    DisplayVersion = "1.0"
        ...
        } -ModuleName SoftwareDetection -ParameterFilter { $Path -like "*Microsoft\Windows\CurrentVersion\Uninstall\*" }

        # Mock par défaut pour Get-ItemProperty
        Mock Get-ItemProperty {
            Write-Debug "Mock par défaut Get-ItemProperty appelé pour Path: $Path"
            return @()
        } -ModuleName SoftwareDetection

        # Mock pour Get-ChildItem (portables)
        Mock Get-ChildItem {
            Write-Debug "Mock Get-ChildItem (portables) appelé pour Path: $Path"
            return @(
                [PSCustomObject]@{
        ...
        Mock Get-ChildItem {
            Write-Debug "Mock Get-ChildItem (raccourcis) appelé pour Path: $Path"
            return @(
                [PSCustomObject]@{
            ...
            $Path -eq "C:\ProgramData\Microsoft\Windows\Start Menu\Programs" -or 
            $Path -eq "C:\Users\DERFB\Desktop" -or 
            $Path -eq "C:\Users\DERFB\AppData\Roaming\Microsoft\Windows\Start Menu\Programs"
        }

        # Mock pour WScript.Shell
        Mock New-Object {
            Write-Debug "Mock New-Object appelé pour TypeName: $TypeName"
            return [PSCustomObject]@{
                CreateShortcut = {
                    param($Path)
                    Write-Debug "Mock CreateShortcut appelé pour Path: $Path"
                    return [PSCustomObject]@{
        ...
        Mock Get-FileVersionSafely {
            Write-Debug "Mock Get-FileVersionSafely appelé pour FilePath: $FilePath"
            return @{
                ProductName = "TestApp"
                ProductVersion = "1.0"
        ...
        Mock Test-PathSafely {
            Write-Debug "Mock Test-PathSafely appelé pour Path: $Path"
            return $true
        ...
        Mock New-SoftwareApp {
            ...
            Write-Debug "Mock New-SoftwareApp appelé pour Name: $Name"
            return [PSCustomObject]@{
                Name = $Name
                Version = $Version
                Publisher = $Publisher
        ...
        Mock Remove-DuplicateSoftware {
            ...
            Write-Debug "Mock Remove-DuplicateSoftware appelé avec SoftwareList: $($SoftwareList | ConvertTo-Json -Depth 2)"
